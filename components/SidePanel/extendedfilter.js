...
import { normalize, rowsToGraph } from '../../util'
...
const ExtendedFilter = ({
  ...
  const handleFilters = (label, value) => {
    setFilters((f) => {
      ...
      if (label === 'state') {
        ...
        updatePatients(
          normalize(
            rawPatients.filter((x) => x.state == value),
            false
          )
        )
        var newGraph = rowsToGraph(
          rawPatients.filter((x) => x.state == value),
          false,
          false
        )
        ...
        updatePatients(
          normalize(
            rawPatients.filter((x) => x.district == value),
            false
          )
        )
        updateGraph(
        ...
        updatePatients(
          normalize(
            rawPatients.filter((x) => x.city == value),
            false
          )
        )
        updateGraph(
  ...
  function getSortedValues(obj, key) {
    const setValues = new Set(obj && obj.map((p) => p[key]))
    let returnValue = Array.from(setValues)
      .sort()
      .map((x, i) => {
  ...
  return (
    <div className="filters-left">
      <br></br>
      <Title>State, District and city filter:</Title>
      <br></br>
      <div className="select">
        ...
        />
        <div id="dvdistrict" style={{ display: 'none', marginTop: '10px' }}>
          <Select
            id="district"
            onChange={(value) => {
        ...
        <div id="dvcity" style={{ display: 'none', marginTop: '10px' }}>
          <Select
            id="city"
            onChange={(value) => {
